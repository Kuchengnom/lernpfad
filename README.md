...
Open `http://127.0.0.1:4173/lernen.html` for the app; `/` is the parent landing page. Wait for **Offline bereit** after initial loading. The production app caches its own shell, so you can reload and practise with the network disconnected. Use HTTPS when deploying elsewhere; `file://` is not supported.

## Use the app

- Start a short practice round from **Heute**. Wrong answers return in future practice; completed rounds award effort points and count toward six collectible stamp milestones. Leaving a round or reloading retains answered progress and lets you resume.
...
- **Üben → Lernstoff hinzufügen** offers **Text einfügen** for a copied ChatGPT JSON answer and **Datei auswählen** for a file. Both open the same preview of curriculum JSON or a complete backup. Pasted text can be raw JSON or one complete Markdown `json` code block. The draft survives editing, errors and preview cancellation within the open tab; it is not saved across reloads. Review its file name, content, sample tasks, sources and prerequisite warnings; export the current backup directly from the preview. Only **Lernstoff übernehmen** applies the preview. A new course or version adds a separate learning book; an identical curriculum opens its existing book with progress intact. Changed content with the same ID/version is rejected. A single-book backup explicitly replaces only its matching book’s progress. Cancel or leave the preview to keep everything unchanged.
- **Meine Lernbücher** switches between independently saved books and paused rounds. Titles can be renamed locally; source hints, import dates and practice dates help identify them.
- **Alles sichern** downloads the full profile: all books, learner outcomes, local titles and collected stamps. Import with an explicit replacement preview to restore on another device. Answered outcomes are portable; an unfinished round starts anew after backup restore. Single-book and curriculum-only exports remain available.
- **Sammelmappe** displays six original motifs earned after 1, 3, 5, 10, 20 and 40 completed rounds across books. Earned stamps survive switching and earlier single-book backup restores.
- The mountain route shows completed tasks. Rounds of at least eight tasks offer a voluntary **Bergzeit** at the midpoint, with a persistent pause and no countdown.
- **Vorlesen** uses a matching voice explicitly reported as local by the device. When unavailable, the app explains this; it never silently selects a remote/default voice. Real iPhone voice availability and flight-mode playback still require a device test.
