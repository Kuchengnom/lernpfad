export function mountainRoute(state) {
  ...
  return `<div class="mountain-progress" role="img" aria-label="${complete} von ${total} Aufgaben geschafft"><svg viewBox="0 0 280 80" aria-hidden="true"><polyline points="${points.map(p => p.join(',')).join(' ')}" class="mountain-line"/>${points.map(([x,y], i) => `<circle cx="${x}" cy="${y}" r="${i === complete ? 6 : 3}" class="${i <= complete ? 'waypoint-done' : 'waypoint-open'}"/>`).join('')}<path d="M268 19V3l-12 4 12 4" class="summit-flag"/></svg><span>${complete} von ${total} geschafft</span></div>`;
}
export function restView(state, button) {
  return `<section class="rest-page" aria-labelledby="rest-title"><p class="eyebrow">Bergzeit</p><h1 id="rest-title">Einmal kurz verschnaufen.</h1><img class="rest-art" src="./illustrations/lernpfad-bergzeit.webp" alt="Eine Bank mit Aussicht auf ein ruhiges Bergtal" width="1200" height="800"><p class="lede">Du hast ${state.index} Aufgaben geschafft. Noch ${state.session.exercises.length - state.index} bis zum Ziel.</p><p>Schau kurz in die Ferne und lass die Schultern locker. Du entscheidest, wann es weitergeht.</p><div class="rest-actions">${button('Weiterwandern', 'continue-rest', 'button button--primary')}${button('Später fortsetzen', 'exit', 'button button--quiet')}</div></section>`;
}
