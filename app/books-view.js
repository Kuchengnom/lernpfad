...
function bookCard(book, index, state, esc, button) {
  ...
  return `<article class="card book-card ${selected ? 'book-card--selected' : ''}" aria-labelledby="book-title-${index}">
    ...
    <p class="book-card__meta">${imported ? `Hinzugefügt am ${esc(imported)}` : 'Datum nicht erfasst'}<br>${lastPractised ? `Zuletzt geübt am ${esc(lastPractised)}` : 'Noch keine Antwort gespeichert'}</p>
...
export function booksView(state, esc, button) {
  ...
  return `<section class="books-page" aria-labelledby="books-title">
    ...
    <section class="card books-examples" aria-labelledby="books-examples-title"><div><p class="eyebrow">Schnell testen</p><h2 id="books-examples-title">Beispiel-Lernbücher hinzufügen</h2><p>Füge Mathe oder Französisch einzeln über die gewohnte Vorschau hinzu. Dein aktuell ausgewähltes Buch bleibt geöffnet.</p></div><div class="books-examples__actions">${button('Französisch-Beispiel hinzufügen', 'load-example', 'button button--secondary', 'data-example="french" data-keep-active-book="true"')}${button('Mathe-Beispiel hinzufügen', 'load-math-example', 'button button--secondary', 'data-keep-active-book="true"')}</div></section>
    <div class="books-toolbar"><p>${books.length} ${books.length === 1 ? 'Lernbuch' : 'Lernbücher'}</p><div>${button('Text einfügen', 'navigate', 'button button--secondary', 'data-view="import-text"')}${button('Zur Übersicht', 'navigate', 'button button--secondary', 'data-view="library"')}</div></div>
    <div class="books-grid">${books.map((book, index) => bookCard(book, index, state, esc, button)).join('') || '<div class="empty-state"><h2>Platz für dein erstes Lernbuch</h2><p>Füge Lernstoff als Text ein oder wähle eine JSON-Datei in der Übersicht.</p></div>'}</div>
