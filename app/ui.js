...
import { learnerText, isMath, contentLanguage, subjectName, curriculumLanguageLabel } from './study-language.js';
...
function library(state) {
  return shell(state, `<section class="utility-page" aria-labelledby="library-title">
    ...
    <div class="import-preview__actions">${button('Meine Lernbücher', 'navigate', 'button button--secondary', 'data-view="books"')}${button('Sammelmappe', 'navigate', 'button button--secondary', 'data-view="album"')}${button('Alles sichern', 'export-profile', 'button button--quiet')}</div><div class="library-handoff">
      <article class="card utility-card"><div class="utility-icon">${icons.archive}</div><h2>Lernstoff hinzufügen</h2><p>Kopiere die JSON-Antwort aus ChatGPT hierher oder wähle eine Datei. Erst in der Vorschau übernimmst du den neuen Lernstoff.</p><div class="import-entry-actions">${button('Text einfügen', 'navigate', 'button button--primary', 'data-view="import-text"')}<label class="button button--secondary file-button">Datei auswählen<input type="file" accept="application/json,.json" data-import-file></label></div></article>
      <div class="card authoring-entry scout-authoring-entry"><div class="authoring-entry__copy"><h2>Eigenen Lernstoff erstellen</h2><p>Aus deinem Schulmaterial wird dein eigener Lernpfad. Mit Anleitung und fertiger Anweisung für den KI-Chat, für Sprachen oder Mathematik.</p>${button('Lernstoff erstellen', 'navigate', 'button button--primary', 'data-view="authoring"')}</div>${fieldKitArtwork()}</div>
    </div>
...
function importSourceText(source) {
  ...
  // ponytail: source.file is a repo-relative path (e.g. references/textbook/…) meant for the curriculum author,
  // not this non-technical audience — show only what's useful to a parent (kind, page, notes).
  return [kind, source.page ? `Seite ${source.page}` : '', source.notes].filter(Boolean).join(' · ');
}
...
function importPreview(state) {
  ...
  return shell(state, `<section class="import-preview" aria-labelledby="import-preview-title">
    <p class="eyebrow">Lernstoff prüfen</p>
