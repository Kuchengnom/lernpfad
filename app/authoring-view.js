...
export function authoringView(state, esc, button) {
  ...
  return `<section class="authoring-page" aria-labelledby="authoring-title">
    ...
    <ol class="authoring-steps">
      <li><strong>Fach wählen und Prompt kopieren.</strong> Er enthält die Anleitung und das vollständige Dateiformat. Eine zusätzliche Schema-Datei ist nicht nötig.</li>
      <li><strong>Neue Unterhaltung öffnen.</strong> Füge den Prompt ein und hänge deine passenden Schulmaterialien an. Ergänze Klassenstufe und gewünschten Umfang. Lernpfad überträgt selbst keine Dateien.</li>
    ...
    <div class="card authoring-tools">
      ...
      <p>Erklärungen und Aufgabenstellungen bleiben auf Deutsch. Der gewählte Prompt ist für <strong>${math ? 'Mathematik · Teilbarkeit &amp; Primzahlen' : language === 'fr' ? 'Französisch' : 'Englisch'}</strong>.</p>
      ...
      <details class="import-preview__details" data-authoring-details><summary>Prompt ansehen oder manuell kopieren</summary><label for="authoring-prompt">Vollständiger Prompt mit Schema</label><textarea id="authoring-prompt" data-authoring-prompt readonly rows="14" spellcheck="false">${esc(buildAuthoringPrompt(language, subject))}</textarea></details>
    </div>
